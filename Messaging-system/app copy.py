...
def messages():
	if request.args.get('thread_id'):
		...
		message_thread_sender = database.session.query(Message).filter(Message.thread_id == request.args.get('thread_id'), Message.sender_id == current_user.username, Message.sender_del == False)
		message_thread_recipient = database.session.query(Message).filter(Message.thread_id == request.args.get('thread_id'), Message.recipient_id == current_user.username, Message.recipient_del == False)
		...
		if request.args.get('fetch'): 
			...
			testq = message_thread_sender.union(message_thread_recipient).order_by(Message.timestamp.asc()).filter(Message.message_id < fetch_last_query.id) 
		...
		if page > 1:
			paged_threads = render_template('new_thread.html', messages=message_threads.items, unread_threads_list=unread_threads_list)
...
def message_delete():
	...
	if request.json.get('type') == 'thread':
		messages = database.session.query(Message).filter(Message.thread_id == request.json.get('url')).all()
		...
		sender_messages = database.session.query(Message).filter(Message.thread_id == message.thread_id, Message.sender_id == current_user.username, Message.sender_del == False)
		recipient_messages = database.session.query(Message).filter(Message.thread_id == message.thread_id, Message.recipient_id == current_user.username, Message.recipient_del == False)
	...
	if request.json.get('type') == 'message':
		message = database.session.query(Message).filter(Message.url == request.json.get('url')).first()
